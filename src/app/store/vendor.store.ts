...
import { Vendor, VendorFilter, VendorContact, VendorProduct } from '../models';
import { UserStore, ProductStore, ProductVendorStore } from './';
...
export class VendorStore {
  ...
  // ── Computed: vendors filtered by selected location ───────────
 ...
 /** Resolved products for the selected vendor (joins catalog + links) */
  readonly productsForVendor = computed((): VendorProduct[] => {
    const vid = this.selectedVendorId();
    if (!vid) return [];

    const links = this.productVendorStore.links().filter(l => l.vendorId === vid);
    const catalog = this.productStore.products();
    // const counts = this.vendorCountByProduct();

    return links
      .map(link => {
        const product = catalog.find(p => p.productCode === link.productCode);
        ...
        return {
          ...
          vendorId: vid,
          //vendorCount: counts.get(product.code) ?? 1,
...
// ═══════════════════════════════════════════════════════════════
  ...
  vendorSelected(vendorId: string): void {
  ...
  getPrimaryContact(vendorId: string) {
    const v = this.getVendorById(vendorId);
    return v?.contacts.find((c) => c.isPrimary) ?? v?.contacts[0] ?? null;
  }
