...
export class ProductStore {
  ...
  readonly lowStockProducts = computed(() =>
    this._products().filter((p) => p.reorderLevel !== null && p.unitsAvailable <= p.reorderLevel),
  );
  ...
  getProductById(id: string): Product | undefined {
   ...
   getProductByCode(code: string): Product | undefined {
    return this._products().find((p) => p.productCode === code);
  }
  ...
  //         preferredVendorId: p.preferredVendorId === vendorId ? null : p.preferredVendorId,
  //         updatedAt: new Date().toISOString(),
  //       };
  //     }),
  //   );
